...
- Команды: `/start`, `/menu`, `/help`, `/shop`, `/plant`, `/farm`, `/harvest`, `/inventory`, `/expedition`, `/tools`, `/balance`, `/level`
...
- Инвентарь
- Live-обновление карточек кнопкой `🔄 Обновить`
- Главное меню как игровой хаб: постоянная Reply-клавиатура + inline-навигация внутри экранов
- Экспедиция: энергия, регенерация и случайная награда (монеты/семена/подарок)
...
- Нижняя Reply-клавиатура разделов: `🚜 Ферма`, `🏪 Рынок`, `🎒 Рюкзак`, `🧭 Экспедиция`, `👤 Профиль`.
...
- Базовый лимит: `5/5` энергии.
- 1 экспедиция тратит 1 энергию.
- Автовосстановление: +1 энергия каждые 20 минут.
- При нуле энергии кнопка заранее меняется на `❌ В экспедицию`.
